...
if ( ! class_exists( 'ZOOM_Customizer' ) ) :
    class ZOOM_Customizer {
        public function __construct() {
            ...
            add_action( 'customize_preview_init', array( $this, 'foodica_customizer_js' ), 100 );
        }
        ...
        function foodica_customizer_js()
        {
            wp_enqueue_script(
                'foodica-vein-js',
            ...
            wp_enqueue_script('typekit-font-loader',
                'https://ajax.googleapis.com/ajax/libs/webfont/1.6.16/webfont.js'
            );

            wp_enqueue_script(
                'foodica-customizer-js',
                get_stylesheet_directory_uri() . '/js/customizer-preview.js',
                array('jquery', 'customize-preview', 'underscore'),
                false,
                true // In the footer!
            );
            wp_localize_script('foodica-customizer-js', 'foodica_css_rules', foodica_get_css_rules());
            ...
            foreach ( $args as $setting_id => $option ) {
                ...
                if ( isset( $option['setting'] ) ) {
                    $defaults = array(
                        ...
                        'theme_supports'       => '',
                        'default'              => foodica_get_default( $setting_id ),
                        'transport'            => 'refresh',
                        'sanitize_callback'    => '',
        ...
        public function display_customization() {
            do_action( 'foodica_css' );

            $css = foodica_get_css()->build();
