...
function foodica_logo() {
    $info = foodica_get_logo_information();
    ...
    if ( get_theme_mod( 'logo-retina-ready', foodica_get_default( 'logo-retina-ready' ) ) ) {
        $width /= 2;
        $height /= 2;
    }
 ...
 */
function foodica_has_logo() {
    $information = foodica_get_logo_information();
 ...
 */
function foodica_get_logo_information( $force = false ) {
    $logo_information = array();

    $logo_information['image'] = get_theme_mod( 'logo', foodica_get_default( 'logo' ) );

    if ( ! empty( $logo_information['image'] ) ) {
        $dimensions = foodica_get_logo_dimensions( $logo_information['image'], $force );
...
function foodica_get_logo_dimensions( $url, $force = false ) {
    // Build the cache key
    $key = 'foodica-' . md5( 'logo-dimensions-' . $url . WPZOOM::$themeVersion );
    ...
    if ( false === $dimensions || is_preview() || true === $force ) {
        ...
        $attachment_id = foodica_get_attachment_id_from_url( $url );
 ...
 */
function foodica_get_attachment_id_from_url( $url = '' ) {
 ...
 * @return void
 */
function foodica_display_favicons() {
    $logo_favicon = get_theme_mod( 'logo-favicon', foodica_get_default( 'logo-favicon' ) );
    if ( ! empty( $logo_favicon ) ) : ?>
        <link rel="icon" href="<?php echo esc_url( $logo_favicon ); ?>"/>
    <?php endif;
}

add_action( 'wp_head', 'foodica_display_favicons' );
